                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-8">
                                        <router-link :to="{name: 'AuditorPrint'}" target='_blank' class="btn btn-danger mt-2">
                                        ...
                                        <div data-app>
                                            <v-data-table :headers="headers" :items="rows"
                                                ...
                                                <template v-slot:item.actions="{ item }">
                                                    <v-btn color="bg-success" class="mx-2" dark fab
                                                        small @click="modalUpdate(item)">
                                                        <v-icon>
                                                            mdi-pencil
                                                        </v-icon>
                                                    </v-btn>
                                                    <v-btn color="bg-danger" dark fab small
                                                        @click="handledelete(item.id_evaluasi)">
                                                        <v-icon>
                                                            mdi-delete
                                                        </v-icon>
                                                    </v-btn>
  ...
  components: { NavAuditor },
        ...
        methods: {
            ...
            handleUpdate() {
                ...
                axios.put('/api/auditor/audit/'+this.form.id_evaluasi, this.form).then((response) => {
                    if (response.data.status) {
                        this.$swal.fire({
                            toast: true,
                            icon: 'success',
            ...
            handledelete(id) {
                this.$swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete('/api/auditor/audit/' +id).then((response) => {
                            if(response.data.status) {
                                this.$swal.fire({
                                    icon: 'success',
